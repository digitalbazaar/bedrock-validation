 ...
 */
export function validate(name, data, callback) {
  ...
  Object.keys(options).forEach(key => {
    schemas[key] = getSchema(options[key]);
  ...
  if(arguments.length > 1) {
    ...
    return validateInstance(data, schemas.body, callback);
  }

  // pre-compile schemas
  let queryFn;
  if(schemas.query) {
    queryFn = compile(schemas.query);
    queryFn.title = schemas.query.title;
  }
  let bodyFn;
  if(schemas.body) {
    bodyFn = compile(schemas.body);
 ...
 * @return the validation result.
 */
export function validateInstance(instance, schema, callback) {
...
export function createValidateMiddleware({bodySchema, querySchema}) {
  ...
  if(bodySchema) {
    validateBodySchema = compile(bodySchema);
  }
  let validateQuerySchema;
  if(querySchema) {
    validateQuerySchema = compile(querySchema);
  }
