...
api.validate = function(name, data, callback) {
  ...
  let queryFn;
  if(schemas.query) {
    queryFn = ajv.compile(schemas.query);
    queryFn.title = schemas.query.title;
  }
  let bodyFn;
  if(schemas.body) {
    bodyFn = ajv.compile(schemas.body);
  ...
  return (req, res, next) => {
    if(schemas.query) {
      const result = api.validateInstance(req.query, queryFn);
      if(!result.valid) {
        return next(result.error);
      }
    }
    if(schemas.body) {
      const result = api.validateInstance(req.body, bodyFn);
      if(!result.valid) {
        return next(result.error);
      }
    }
